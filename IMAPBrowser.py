...
of the emails.
'''
dataset_path = r"G:\Bielefeld\Individual Project\DataFrame.csv"

'''
path of the pickle dataset. It contains the details of the nodes in the H2 
graph.
'''
pickle_dataset_path = r"G:\Bielefeld\Individual Project\TreeDataFrame.pkl"
...
class Login:
    ...
    def get_credentials(self):
        ...
        Please use windows CMD to run the script so as not to echo the password
        or built-in terminal from PyCharm.
...
class ImapParse:
        ...
        if not self.sync:
            ...
            for node in content:
                ...
                if node.name == 'Root':
                    ...
                    adjacency_list.append((node.parent.number - 1, 
                                           node.number - 1))
            ...
            for node in directories:
                ...
                if date > node.timestamp:
                    ...
                    print('There are no new mails in ' + node.name + ' to be \
                          synced.')
            return
    ...
    def check_for_emails(self, node):
        ...
        If the status does not get changed the same mail would be fetched again
    ...
    def if_immediate_child(self, child, parent):
        """
    ...
    def get_mail(self, node, data):
        ...
        '''
        During synchronization when a new record was getting inserted the index
        of the new item always turned out 1.
        To fix the issue, first we read the maximum value of the column 'Index'.
        The new index thus would be the returned value of the Index + 1.
        '''
        if self.sync:
        ...
        for num in data[0].split():
            ...
            if recent_mail and not self.sync:
                ...
                '''
                During synchronization mails cannot be sorted out in descending
                order of date.
    ...
    def get_mail_size(self, num):
        ...
        return lst[-1].decode('utf-8')[last_whitespace_index - 
                  len(byte_array) + 1:-1]
    ...
    def get_converted_timestamp(self, date):
        ...
        '''
        convert the date string into an integer, so that Date operations can be
        performed on it.
        '''
        ...
        format.
        '''
        timestamp = datetime.datetime(int(timestamp[4:8]), int(timestamp[2:4]),
                                      int(timestamp[:2]), int(timestamp[8:10]),
                                      int(timestamp[10:12]), int(timestamp[12:]))
        return timestamp

    def get_timestamp_range(self, year):
        """
...
class ImapTree:
    ...
    def grow(self, node, directory, ismailnode=False, sync=False):
        ...
        if sync:
            ...
            dataset.
            A new node addition means the number of the new node should be
            the length of the list incremented by 1.
            '''
...
class H2Tree:
    ...
    def hyperbolize(self, node):
        ...
        if not self.reposition:
            ...
            pos_children = self.rs[node.depth] * np.exp(1j * \
                                  linspace(-self.phi_0s[node.depth],
        ...
        for i, child in enumerate(node.children):
            if self.reposition:
                ...
                self.position_dict[child.number] = (pos_children[i].real, 
                                  pos_children[i].imag)
        ...
        while depth > 0:
            for node in self.pickle_dataset:
                if node.depth == depth:
                    ...
                    if node.isMail:
                        node.parent.numberOfMails = node.parent.numberOfMails \
                        + 1
    ...
    def render_h2_tree(self, positions):
        ...
        '''
        g2 and g3 are graph items
        They would represent two semicircles of the same circle
    ...
    def plot_poincare_disc(self, graph_item_1, graph_item_2):
        """
...
if __name__ == '__main__':
    ...
    '''to store the adjacency links between two nodes in the graph'''
    adjacency_list = []
    
    nodeText = []  # to store the labels assigned to the nodes in the graph
    ...
    '''
    the dictionary would be used while converting the timestamp of mails.
    ...
    system.
    The nodes are stored in the form of a dictionary with key as the node 
    number and the position as value.
    '''
    ...
    in the panda dataset,
    else start downloading all the details from the IMAP server.
    '''
    if not os.path.isfile(dataset_path):
        ...
        copied_dataframe = r"G:\Bielefeld\Individual Project\DataFrame_copy.csv"        
        copyfile(original_dataframe, copied_dataframe)
        
        original_pickle = pickle_dataset_path
        copied_pickle = r"G:\Bielefeld\Individual Project\TreeDataFrame_copy.pkl"
        copyfile(original_pickle, copied_pickle)
        
        '''
        In certain cases, when the script connects to the IMAP server and tries
        to synchronize an error occurs.
        This error could be due to issues in the script, a wrong action trying
        to be executed on the server, etc.
        Due to any such issue, the script may/may not update partial details 
        in the DataFrame.csv file and exit. This leads to inconsistencies with 
        the TreeDataFrame.pkl file.
        In such cases, the script replaces the newly updated DataFrame.csv file
        with the last correct version of itself. Similarly, to ensure that 
        TreeDataFrame.pkl is also consistent, it is also replaced by the last 
        ...
        try:
            ...
            print('\nAn error occurred during synchronization call to the \
                  IMAP server. The last versions of the dataframes have been \
                  succesfully restored.\n')
            print('Please fix the issues occuring and re-run the script.\n')
            print('Exiting....')
        ...
        '''
        list of the labels of various nodes retrieved from pickle dataset after
    ...
    '''
    Create an instance of the H2tree class.
    This object would be used to render the H2 tree graph
    '''
    ...
    file system.
    If there is no .pkl file dump the content to .pkl file.
    Based on the content of .pkl file the size of the nodes and the link widths
    would be modified.
    '''
